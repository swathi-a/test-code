...
# Grid-based Sampling
def grid_sampling(n_samples, bounds, optimal_value):
    # Create a grid based on the number of samples
    grid_size = int(np.sqrt(n_samples))
    x1_samples = np.linspace(bounds[0][0], bounds[0][1], grid_size)
    x2_samples = np.linspace(bounds[1][0], bounds[1][1], grid_size)
    X1, X2 = np.meshgrid(x1_samples, x2_samples)
    
    # Stack the grid into a list of sample points
...
# Fit Gaussian Process for Grid Sampling
...
def main():
    ...
    # Grid-based Sampling
    print("Running Grid-based Sampling...")
    X_grid, y_grid, grid_regrets, grid_cumulative_regrets = grid_sampling(n_iterations, bounds.numpy(), optimal_value)
    gp_grid = fit_gaussian_process(X_grid, y_grid)
    plot_estimated_mean_variance(gp_grid, bounds.numpy())
    plot_regret_cumulative_regret(grid_regrets, grid_cumulative_regrets, 'Grid Sampling')
    plot_contour_with_sampled_points(X_grid, 'Grid Sampling')
