...
problem = Ackley(dim=3, negate=True)
bounds = torch.tensor([[-5.0, -5.0, -5.0], [10.0, 10.0, 10.0]])
optimal_value = 0.0  # Known global minimum of the Ackley function at [0, 0]
optimal_point = np.array([0.0, 0.0, 0.0])  # Optimal point for Ackley function

# Random Sampling
def random_sampling(n_iter=20):
    try:
        random_x = torch.rand(n_iter, 3) * (bounds[1] - bounds[0]) + bounds[0]
...
def bayesian_optimization(n_iter=20, n_initial_points=5):
    try:
        train_x = torch.rand(n_initial_points, 3) * (bounds[1] - bounds[0]) + bounds[0]
...
def tpe_optimization(n_iter=20):
    try:
        def objective(trial):
            ...
            x3 = trial.suggest_uniform('x3', -5.0, 10.0)
        ...
        tpe_x = torch.tensor([[trial.params['x1'], trial.params['x2'], trial.params['x3']] for trial in study.trials])
