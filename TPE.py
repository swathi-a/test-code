...
optimal_point = np.array([9.42478, 2.475])  # Optimal point for Branin function
...
def plot_results(random_x, bayes_x, tpe_x, random_regrets, bayes_regrets, tpe_regrets):
    ...
    # Plot contour for Branin function
    X1_range = torch.linspace(-5, 10, 100)
    X2_range = torch.linspace(0, 15, 100)
    ...
    plt.ylabel('x2')
    plt.legend()
    plt.title('Contour plot of Branin function with Optimization points')
    plt.savefig("contour_plot.png", dpi=300)
    plt.show()

    # 2D Exploration of x1 and x2 for each method
    fig, axs = plt.subplots(3, 1, figsize=(12, 12))

    axs[0].scatter(random_x[:, 0].numpy(), random_x[:, 1].numpy(), color='red')
    axs[0].set_xlabel('x1')
    axs[0].set_ylabel('x2')
    axs[0].set_title('Random sampling Exploration')

    axs[1].scatter(bayes_x[:, 0].numpy(), bayes_x[:, 1].numpy(), color='blue')
    axs[1].set_xlabel('x1')
    axs[1].set_ylabel('x2')
    axs[1].set_title('Bayesian Optimization Exploration')

    axs[2].scatter(tpe_x[:, 0].numpy(), tpe_x[:, 1].numpy(), color='green')
    axs[2].set_xlabel('x1')
    axs[2].set_ylabel('x2')
    axs[2].set_title('TPE Optimization Exploration')

    plt.savefig("exploration_plots.png", dpi=300)
    plt.tight_layout()
    plt.show()
